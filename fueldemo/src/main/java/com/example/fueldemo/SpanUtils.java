...
public final class SpanUtils {
    ...
    /**
     * 以小数点为基准,设置字体大小, 例如: ¥200.oo
     *
     * @param amount 背景色
     * @param before 小数点前面的数字大小
     * @param after  小数点后面的数字大小
     * @return {@link SpannableStringBuilder}
     */
    public SpannableStringBuilder setDecimalPointFontSize(Context context, String amount, int before, int after) {
        SpannableStringBuilder builder = new SpannableStringBuilder(amount);
        int beforeVal = (int) DpUtil.sp2px(before);
        int afterVal = (int) DpUtil.sp2px(after);
        try {
            builder.setSpan(new AbsoluteSizeSpan(beforeVal), 0, amount.indexOf(".")
                    , Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            builder.setSpan(new AbsoluteSizeSpan(afterVal), amount.indexOf("."), amount.length()
                    , Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     * @return {@link SpannableStringBuilder}
     */
    public SpannableStringBuilder setDecimalPointFontSize02(Context context, String amount, int before, int after) {
        SpannableStringBuilder builder = new SpannableStringBuilder(amount);
        int beforeVal = (int) DpUtil.sp2px(before);
        int afterVal = (int) DpUtil.sp2px(after);
        try {
            if (amount.contains(".")) {
                ...
                builder.setSpan(new AbsoluteSizeSpan(beforeVal), 0, amount.indexOf("云")
                        , Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                builder.setSpan(new AbsoluteSizeSpan(afterVal), amount.indexOf("云"), amount.length()
                        , Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void updateCharCharSequence() {
        ...
        if (iconMarginGapWidth != -1) {
            if (iconMarginBitmap != null) {
                ...
                mBuilder.setSpan(new CustomIconMarginSpan(Cxt.get(), iconMarginUri, iconMarginGapWidth, alignIconMargin), start, end, flag);
            } else if (iconMarginResourceId != -1) {
                mBuilder.setSpan(new CustomIconMarginSpan(Cxt.get(), iconMarginResourceId, iconMarginGapWidth, alignIconMargin), start, end, flag);
            }
        }
        if (fontSize != -1) {
    ...
    private void updateImage() {
        ...
        int end = start + 5;
        if (imageBitmap != null) {
            mBuilder.setSpan(new CustomImageSpan(Cxt.get(), imageBitmap, alignImage), start, end, flag);
        } else if (imageDrawable != null) {
            mBuilder.setSpan(new CustomImageSpan(imageDrawable, alignImage), start, end, flag);
        } else if (imageUri != null) {
            mBuilder.setSpan(new CustomImageSpan(Cxt.get(), imageUri, alignImage), start, end, flag);
        } else if (imageResourceId != -1) {
            mBuilder.setSpan(new CustomImageSpan(Cxt.get(), imageResourceId, alignImage), start, end, flag);
        }
    }
    ...
    /**
     * @Description: 用来设置字体大小和对齐方式
     * @Author: 秦王
     * @Copyright: 浙江集商优选电子商务有限公司
     * @CreateDate: 2019/4/9 18:59
     * @Version: 1.0.0
     */
    class VerticalCenterWithFontSizeSpan extends ReplacementSpan {
        /**
         * DES: 文本大小
         */
         ...
         * @param textSize 文本大小
         **/
        VerticalCenterWithFontSizeSpan(float textSize) {
            mTextSize = textSize;
        }
        ...
        public int getSize(Paint paint, CharSequence text, int start, int end, Paint.FontMetricsInt fm) {
            ...
            initTextPaint(paint);
            // DES: 测量文本大小
            return (int) mTextPaint.measureText(text.toString());
        }
        ...
        public void draw(Canvas canvas, CharSequence text, int start, int end, float x, int top, int y, int bottom, Paint paint) {
            ...
            canvas.drawText(
                    text.toString(),
                    x,
                    getNewTextY(mTextPaint.getFontMetricsInt(), top, y, bottom),
                    mTextPaint);
        }
        ...
        private void initTextPaint(Paint paint) {
            ...
            // DES: 设置文本大小
            mTextPaint.setTextSize(DpUtil.dp2px(mTextSize));
        }
         ...
         * TIME: 2019/4/9 19:23
         *
         * @param fontMetricsInt FontMetrics实例
         * @param top            顶部距离
         * @param y              原始Y位置
         * @param bottom         底部位置
         **/
        private float getNewTextY(Paint.FontMetricsInt fontMetricsInt, int top, int y, int bottom) {
            return y - ((y + fontMetricsInt.descent + y + fontMetricsInt.ascent) / 2f - (bottom + top) / 2f);
        }
    }

    /**
     * DES: 设置文本大小并且是竖直居中的相对于前面的文本
     * AUTHOR: 秦王
     * TIME: 2019/4/9 19:27
     *
     * @param textSize 文本大小，单位dp
     **/
    public SpanUtils setFontSizeAndVerticalCenter(@FloatRange(from = 10) float textSize) {
        setSpans(new VerticalCenterWithFontSizeSpan(textSize));
        return this;
    }
}
